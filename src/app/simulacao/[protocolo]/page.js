...
export default function ShowSimulation() {
    ...
    useEffect(() => {
        async function fetchResults() {
            const response = await getByProtocol(params.protocolo)
                .then((results) => {
                    setSimulation(results.data)
                    setProduct(results.data.rendaFixa)
                    setProfit(results.data.valorFinal - results.data.valorInicial)
                    setRentability((results.data.valorFinal - results.data.valorInicial) / results.data.valorInicial * 100)
    ...
    useEffect(() => {
        async function getAllRendasFixas() {
            const response = await getRendasFixas()
                .then((res) => res.data)
                .catch(() => []);
            ...
            {simulation && (
                <div>
                    ...
                    <div className="max-w-[1152px] mx-auto">
                        ...
                        <h2 className="text-center text-[18px] text-[#5F799F] max-w-2xl mx-auto mb-8">Investidor {simulation.tipoUsuario
